...
- **Free tier vs. persistent data**: the default `render.yaml` targets Render's **Free** plan, which has no persistent disk — the SQLite database and LLM call log fall back to `config.py`'s local defaults, meaning all sessions/questions/reports are wiped on every restart, redeploy, or cold start after inactivity. This is fine for a quick demo, not for real use. To get real persistence: change `plan: free` to `plan: starter` (or above), add back a `disk:` block mounted at e.g. `/var/data`, and point `DB_PATH`/`LLM_CALL_LOG_PATH` at that mount.
...
- **Taxonomy seeding is automatic**: `app.py`'s startup (`lifespan`) calls `seed_taxonomy.seed()` on every boot — idempotent and cheap, so the chapter/concept picker populates itself on first deploy and re-populates itself after every restart/redeploy on plans with no persistent disk. No manual step needed, which matters because **Shell access itself requires Starter plan or above** — Free tier doesn't have a Shell tab to run commands in.
- **Question pool warmup is manual and needs Shell**: `python scripts/warmup_all_chapters.py` pre-generates questions so early users don't wait on live generation, but it makes real LLM calls and can only be run via Render's Shell tab — **not available on Free tier**. On Free tier, the pool starts empty and fills in via the app's existing just-in-time generation fallback as students use it (slower first questions per concept, but functional).
- **Free-tier cold starts**: the service spins down after inactivity; combined with background pool generation, the first request after a cold start may be slower than usual.
